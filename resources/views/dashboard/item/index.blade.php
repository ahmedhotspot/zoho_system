...
@section('title', __('dashboard.items'))
    ...
    <div class="toolbar" id="kt_toolbar">
        ...
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            ...
            <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
                <!--begin::Title-->
                <h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">{{ __('dashboard.items') }}</h1>
                <!--end::Title-->
                <!--begin::Separator-->
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                    ...
                    <!--end::Item-->
                    <!--begin::Item-->
                    <li class="breadcrumb-item text-dark">{{ __('dashboard.items') }}</li>
                    <!--end::Item-->
                </ul>
                <!--end::Breadcrumb-->
            </div>
                        ...
                        <div class="d-flex justify-content-end gap-2" data-kt-customer-table-toolbar="base">
                            ...
                            <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
                                ...
                                <div class="px-7 py-5" data-kt-customer-table-filter="form">
                                    ...
                                    <!--begin::Input group-->
                                    <div class="mb-10">
                                        <label class="form-label fs-6 fw-bold">{{ __('dashboard.item_type') }}:</label>
                                        <select class="form-select form-select-solid fw-bolder" id="filter_item_type">
                                            <option value="">{{ __('dashboard.all') }}</option>
                                            <option value="sales" {{ request('item_type') == 'sales' ? 'selected' : '' }}>{{ __('dashboard.sales') }}</option>
                                            <option value="purchases" {{ request('item_type') == 'purchases' ? 'selected' : '' }}>{{ __('dashboard.purchases') }}</option>
                                            <option value="sales_and_purchases" {{ request('item_type') == 'sales_and_purchases' ? 'selected' : '' }}>{{ __('dashboard.sales_and_purchases') }}</option>
                                            <option value="inventory" {{ request('item_type') == 'inventory' ? 'selected' : '' }}>{{ __('dashboard.inventory') }}</option>
                                        </select>
                                    </div>
                                    <!--end::Input group-->
                                    <!--begin::Input group-->
                                    <div class="mb-10">
                                        <label class="form-label fs-6 fw-bold">{{ __('dashboard.product_type') }}:</label>
                                        <select class="form-select form-select-solid fw-bolder" id="filter_product_type">
                                            <option value="">{{ __('dashboard.all') }}</option>
                                            <option value="goods" {{ request('product_type') == 'goods' ? 'selected' : '' }}>{{ __('dashboard.goods') }}</option>
                                            <option value="service" {{ request('product_type') == 'service' ? 'selected' : '' }}>{{ __('dashboard.service') }}</option>
                                        </select>
                                    </div>
                                    <!--end::Input group-->
                                    <!--begin::Actions-->
                            ...
                            <!--end::Filter-->
                            <!--begin::Sync-->
                            <button type="button" class="btn btn-light-success" id="sync_items">
                                <span class="svg-icon svg-icon-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                ...
                                {{ __('dashboard.sync_from_zoho_items') }}
                            </button>
                            <!--end::Sync-->
                            <!--begin::Add item-->
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5">
                            ...
                            <thead>
                                <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                    <th class="min-w-125px">{{ __('dashboard.item_name') }}</th>
                                    <th class="min-w-100px">{{ __('dashboard.sku') }}</th>
                                    <th class="min-w-100px">{{ __('dashboard.item_type') }}</th>
                                    <th class="min-w-100px">{{ __('dashboard.product_type') }}</th>
                                    <th class="min-w-100px">{{ __('dashboard.rate') }}</th>
                                    <th class="min-w-100px">{{ __('dashboard.unit') }}</th>
                            ...
                            <tbody class="fw-bold text-gray-600">
                                ...
                                <tr>
                                    <!--begin::Name-->
                                    <td>
                                        <a href="{{ route('items.show', $item->id) }}" class="text-gray-800 text-hover-primary mb-1">
                                            {{ $item->name }}
                                        </a>
                                        @if($item->description)
                                        <div class="text-muted fs-7">{{ Str::limit($item->description, 50) }}</div>
                                        @endif
                                    </td>
                                    <!--end::Name-->
                                    <!--begin::SKU-->
                                    ...
                                    <!--begin::Item Type-->
                                    <td>
                                        <span class="badge badge-light-{{ $item->item_type_color }}">
                                    ...
                                    <!--begin::Product Type-->
                                    <td>
                                        <span class="badge badge-light-{{ $item->product_type_color }}">
                                    ...
                                    <!--begin::Status-->
                                    <td>
                                        <span class="badge badge-light-{{ $item->status_color }}">
                                            {{ ucfirst($item->status) }}
                                        </span>
                                    </td>
                                    <!--end::Status-->
                                ...
                                <tr>
                                    <td colspan="8" class="text-center py-10">
                                        <span class="text-muted">{{ __('dashboard.no_items_found') }}</span>
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
...
<script>
    ...
    function applyFilters() {
        ...
        const itemType = document.getElementById('filter_item_type').value;
    ...
    // Sync items
    document.getElementById('sync_items').addEventListener('click', function() {
        const button = this;
